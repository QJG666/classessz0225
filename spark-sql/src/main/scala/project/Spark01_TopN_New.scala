...
object Spark01_TopN_New {
  def main(args: Array[String]): Unit = {
    ...
    // TODO : 实现Top3
    // TODO 1.1 查询用户行为表，过滤出点击行为，和城市以及商品表进行连接查询
    spark.sql(
      """
        |select
        |    c.*,
    ...
    // TODO 1.2 按照地区和商品名称分组，统计出每个商品在每个地区的总点击次数
    spark.sql(
      """
        |select
        |    t1.area,
    ...
    // TODO 1.3 使用排序窗口函数，对每个地区内，按照点击次数降序排序
    spark.sql(
      """
        |select
        |    t2.*,
...
class MyRemarkUDAF extends Aggregator[String, RemarkBuffer, String]{
  ...
  override def finish(buff: RemarkBuffer): String = {
    ...
    // 如果城市的数量大于2
    if ( cityMap.size > 2 ) {
      remarkList = remarkList :+ CityRemark("其它", remarkList.foldLeft(1D)(_ - _.cityRatio))
    }
