...
import com.atguigu.bigdata.spark.project.dao.{HotCategoryAnalysisTop10Dao, HotCategorySessionAnalysisTop10Dao}
...
class HotCategorySessionAnalysisTop10Service extends TService {
  ...
  def analysis1(categories: List[HotCategory]) ={
    ...
    // 使用广播变量提升性能
    val broadcastIds = ProjectUtil.sparkContext().broadcast(cids)
...
//          flg
    ...
    val categoryToSessionSumRDD = categoryToSessionRDD.reduceByKey(_+_)
    ...
    val groupRDD = categoryToSessionSumRDD1.groupByKey()
